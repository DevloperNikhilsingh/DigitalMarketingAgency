...
import { STATUS_OPTIONS } from "../data/dummyData";
...
const ServiceFormModal = ({ open, editingService, onClose, onSubmit }) => {
  ...
  useEffect(() => {
    if (editingService) {
      setForm({
        ...
        iconPreview: editingService.iconPreview || editingService.icon || "",
        title: editingService.title || "",
        category: editingService.category || serviceCategories[0],
        desc: editingService.desc || editingService.description || "",
        tagline: editingService.tagline || "",
        description: editingService.description || "",
        status: editingService.status || "Pending",
        examples:
          Array.isArray(editingService.examples) && editingService.examples.length > 0
            ? editingService.examples.map((ex) => ({
                name: ex.name || "",
                result: ex.result || "",
                image: ex.image || null,
                imagePreview: ex.imagePreview || ex.image || "",
              }))
            : [{ name: "", result: "", image: null, imagePreview: "" }],
      });
    } else {
      setForm(EMPTY_FORM);
    }
  ...
  const validate = () => {
    // Edit mode: no validation, allow saving whatever changed (e.g. just the icon)
    ...
    form.examples.forEach((ex, i) => {
      if (!ex.name.trim() || !ex.result.trim()) {
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-5 flex flex-col gap-5">
          <div className="flex flex-col gap-3.5">
            ...
            <div>
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                <input type="file" accept="image/*,.svg" onChange={handleIconUpload} className="hidden" />
              </label>
            </div>
          ...
          <div className="flex gap-3 pt-1">
            ...
            >
              {editingService ? "Save Changes" : "Upload"}
            </button>
          </div>
        </form>
      </div>
